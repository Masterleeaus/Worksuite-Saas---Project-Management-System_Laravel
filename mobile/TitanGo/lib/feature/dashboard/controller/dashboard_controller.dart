import 'package:titan_go/feature/dashboard/model/booking_statistics_model.dart';
...
class  DashboardController extends GetxController implements GetxService{
  ...
  BookingStatisticsModel? _bookingStatisticsModel;
  BookingStatisticsModel? get bookingStatisticsModel => _bookingStatisticsModel;
  ...
  Future<void> getDashboardData({bool reload = true}) async {
    ...
    if (response.statusCode == 200) {
      ...
      // ── Nexus Field Ops dashboard shape (/api/nexus/v1/dashboard) ─────────
      // { "today_visits": [...], "stage_counts": {...}, "open_issues": N, ... }
      if (body is Map && body.containsKey('today_visits')) {
        final visits = (body['today_visits'] as List?) ?? [];
        for (final v in visits) {
          _booking.add(DashboardBooking.fromJson(v as Map<String, dynamic>));
        }
        // Populate a simple top-cards proxy from Nexus data
        _topCards = TopCards(
          totalBooking:     visits.length,
          totalEarning:     0,
          totalReview:      body['open_issues'] ?? 0,
          completedBooking: visits.where((v) => v['is_complete'] == true).length,
        );
      }
      // ── Legacy Demandium shape (/api/v1/serviceman/dashboard) ────────────
      else if (body is Map && body.containsKey('content')) {
        _topCards = TopCards.fromJson(body['content'][0]['top_cards']);
        _bookings = body['content'][1]['bookings'];
        for (var item in _bookings!) {
          _booking.add(DashboardBooking.fromJson(item));
        }
      }
  ...
  Future<void> getBookingStatisticData({bool isReload = false}) async {

    if(isReload) {
      _bookingStatisticsModel = null;
      update();
    }

    Response response = await dashboardRepository.getBookingStatistic();

    if(response.statusCode == 200){
      _bookingStatisticsModel = BookingStatisticsModel.fromJson(response.body['content']);
