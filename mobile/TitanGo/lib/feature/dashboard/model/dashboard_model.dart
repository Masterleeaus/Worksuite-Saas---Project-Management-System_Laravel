import 'package:titan_go/feature/booking_request/model/booking_request_model.dart';
...
class TopCards {
  ...
  // Extra fields populated from Nexus /api/nexus/v1/dashboard
  int? totalBooking;
  int? totalEarning;
  int? totalReview;
  ...
  TopCards.fromJson(Map<String, dynamic> json) {
    pendingBookings   = int.tryParse(json['total_bookings']?.toString()    ?? '0') ?? 0;
    ongoingBookings   = int.tryParse(json['ongoing_bookings']?.toString()  ?? '0') ?? 0;
    completedBookings = int.tryParse(json['completed_bookings']?.toString() ?? '0') ?? 0;
    canceledBookings  = int.tryParse(json['canceled_bookings']?.toString() ?? '0') ?? 0;
  }
