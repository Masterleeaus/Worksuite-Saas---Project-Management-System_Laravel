...
class AuthController extends GetxController implements GetxService {
  ...
  Future<void> login(String phoneWithCountryCode,String countryCode,String phoneWithoutCountryCode, String password) async {
    ...
    // FSMCore returns: { "token": "...", "worker": { "id": ..., "company_id": ... } }
    // Demandium legacy returns: { "response_code": "auth_login_200", "content": { "token": "..." } }
    // Handle both shapes.
    final bool isFsmResponse = response!.statusCode == 200 && response.body['token'] != null;
    final bool isDemandiumOk = response.statusCode == 200 && response.body['response_code'] == 'auth_login_200';

    if (isFsmResponse || isDemandiumOk) {
      ...
      showCustomSnackBar(response.body['message'].toString().capitalizeFirst ?? response.statusText,
          toasterTitle: 'account_blocked_notice', icon: Images.userBlock);
    } else {
      showCustomSnackBar(response.body['message'].toString().capitalizeFirst ?? response.statusText);
    }
    _isLoading = false;
  ...
  Future<void> _saveDeviceTokenAndNavigate({required Response response, required String phoneWithCountryCode, required String password, required String countryCode}) async {
    ...
    // Extract token — support both FSMCore shape and Demandium legacy shape
    final String? authToken = response.body['token'] as String?
        ?? response.body['content']?['token'] as String?;
    if (authToken == null) return;

    authRepo.saveUserToken(authToken);

    // Extract and persist company_id for tenant-boundary headers
    final Map<String, dynamic>? workerMap = response.body['worker'] as Map<String, dynamic>?;
    if (workerMap != null) {
      final companyId = (workerMap['company_id'] as num?)?.toInt();
      final workerId  = (workerMap['id']          as num?)?.toInt();
      if (companyId != null) {
        Get.find<ApiClient>().setTenantContext(companyId: companyId);
        authRepo.sharedPreferences.setInt(AppConstants.companyIdKey, companyId);
      }
      if (workerId != null) {
        authRepo.sharedPreferences.setInt(AppConstants.workerIdKey, workerId);
      }
    }
