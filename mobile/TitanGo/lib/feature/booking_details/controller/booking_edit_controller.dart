...
class BookingEditController extends GetxController implements GetxService{
  ...
  void onInit() {
    super.onInit();
    initializedControllerValue(Get.find<BookingDetailsController>().bookingDetails?.bookingContent?.bookingDetailsContent
  ...
  void initializedControllerValue(BookingDetailsContent? bookingDetailsContent, {bool shouldUpdate = true}){
    ...
    _initializedCartValue(bookingDetailsContent?.details, bookingDetailsContent);
  ...
  void _initializedCartValue(List<ItemService>? bookedCartList, BookingDetailsContent? bookingDetailsContent){
    _cartList = [];
  ...
  Future<void> updateBooking({String? bookingId, String? subBookingId, String? zoneId, required bool isSubBooking}) async {
    ...
    if(response.statusCode == 200){
      await Get.find<BookingDetailsController>().getBookingDetails( bookingID: subBookingId ?? "", isSubBooking: isSubBooking);
  ...
  void updateCartItemQuantity(CartModel? cart ,int cartIndex, {bool increment = true}) async {
    ...
    String zoneId = Get.find<BookingDetailsController>().bookingDetails?.bookingContent?.bookingDetailsContent?.zoneId ??  Get.find<BookingDetailsController>().bookingDetails?.bookingContent?.bookingDetailsContent?.subBooking?.zoneId ?? "" ;
    ...
    if(quantity <=0  && cart?.isNewItem == true){
      ...
      Response  response  = await  bookingDetailsRepo.getBookingPriceList( zoneId , jsonEncode(variationList));
  ...
  Future<void> getBookingPriceList(String serviceInfo ,{String? thumbnail}) async {
    ...
    Response  response  = await  bookingDetailsRepo.getBookingPriceList( Get.find<BookingDetailsController>().bookingDetails?.bookingContent?.bookingDetailsContent?.zoneId??"" , serviceInfo );
    if(response.statusCode == 200 && response.body["response_code"] == "default_200") {
      bookingPriceList = BookingPrice.fromJson(response.body).bookingPriceContent ?? [];
          ...
          return Theme(
            data: Theme.of(context).copyWith(
              colorScheme: ColorScheme.light(
                ...
                onSurface: Theme.of(context).textTheme.bodyLarge!.color!.withValues(alpha:0.8),
