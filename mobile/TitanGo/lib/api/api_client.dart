...
class ApiClient extends GetxService {
  ...
  /// Tenant boundary fields — populated after successful login.
  /// Sent on every request so the server can validate company context.
  int?    _companyId;
  String? _deviceId;

  ApiClient({required this.appBaseUrl, required this.sharedPreferences}) {
    token      = sharedPreferences.getString(AppConstants.token);
    _companyId = sharedPreferences.getInt(AppConstants.companyIdKey);
    _deviceId  = sharedPreferences.getString(AppConstants.deviceIdKey) ?? _generateDeviceId();
    updateHeader(
      token, [],
  ...
  String _generateDeviceId() {
    ...
    sharedPreferences.setString(AppConstants.deviceIdKey, id);
    return id;
  }

  /// Update headers — called after login and after token refresh.
  /// [companyId] and [deviceId] are optional; only set when they change.
  void updateHeader(String? token, List<int>? zoneIDs, String? languageCode,
      {int? companyId, String? deviceId}) {
    ...
    _mainHeaders = {
      ...
      // X-Company-ID allows the server to validate the token owner's tenant.
      // X-Device-ID  is used for deduplication and offline-sync correlation.
      if (_companyId != null) AppConstants.headerCompanyId: '$_companyId',
      AppConstants.headerDeviceId: _deviceId ?? '',
    };
  }

  /// Update just the tenant IDs after a successful login response.
  void setTenantContext({required int companyId}) {
    _companyId = companyId;
    sharedPreferences.setInt(AppConstants.companyIdKey, companyId);
    updateHeader(token, null, sharedPreferences.getString(AppConstants.languageCode));
  }
