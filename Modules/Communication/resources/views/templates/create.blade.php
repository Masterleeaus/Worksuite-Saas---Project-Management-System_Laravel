...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h2 class="mb-0">
        <i class="bi bi-file-plus me-2"></i>Create Template
    </h2>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('communications.templates.store') }}">
                ...
                <div class="col-sm-4">
                    <select name="type" class="form-select @error('type') is-invalid @enderror" id="tplType">
                        @foreach($types as $key => $label)
                            <option value="{{ $key }}" {{ old('type', 'email') === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mb-3 row" id="subjectRow">
                ...
                <div class="col-sm-10">
                    <input type="text" name="subject" class="form-control @error('subject') is-invalid @enderror" value="{{ old('subject') }}" placeholder="Use {variable} placeholders">
            ...
            <div class="mb-3 row">
                ...
                <div class="col-sm-10">
                    <textarea name="body" rows="10" class="form-control @error('body') is-invalid @enderror" placeholder="Use {customer_name}, {booking_date}, {cleaner_name}, {company_name}…" required>{{ old('body') }}</textarea>
