...
public function store(Request $r){ $data=$r->validate(['amount'=>'required|numeric','expense_category'=>'nullable|integer','description'=>'nullable|string','date'=>'required|date']); Expense::create($data + ['created_by'=>optional($r->user())->id]); return redirect()->route('module-expenses.index')->with('ok','Expense recorded'); }}
