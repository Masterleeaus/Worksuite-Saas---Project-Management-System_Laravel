...
use Modules\BookingModule\Events\CreateQuestion;
use Modules\BookingModule\Events\DestroyQuestion;
use Modules\BookingModule\Events\UpdateQuestion;
...
class QuestionController extends Controller
{
    ...
    {
        if (\Modules\BookingModule\Support\AppointmentPermission::check(Auth::user(), 'question manage')) {
            $question = Question::where('created_by', creatorId())
                ->where('workspace', getActiveWorkSpace())->get();
            return view('bookingmodule::question.index', compact('question'));
        } else {
    ...
    public function create()
    {
        if (Auth::user()->isAbleto('question create')) {
            ...
            return view('bookingmodule::question.create', compact('question', 'question_type'));
        } else {
    ...
    public function edit($id)
    {
        if (Auth::user()->isAbleto('question edit')) {
            ...
            if ($question->created_by == creatorId() && $question->workspace == getActiveWorkSpace()) {
                ...
                $question_type = Question::$question_type;
                return view('bookingmodule::question.edit', compact('question', 'questions', 'question_type'));
            } else {
                return response()->json(['error' => __('Permission denied.')], 401);
            }
        } else {
    ...
    {
        if (\Modules\BookingModule\Support\AppointmentPermission::check(Auth::user(), 'question edit')) {
            if ($question->created_by == creatorId() && $question->workspace == getActiveWorkSpace()) {
                $validator = \Validator::make(
                    $request->all(),
                    [
    ...
    {
        if (\Modules\BookingModule\Support\AppointmentPermission::check(Auth::user(), 'question delete')) {
            $question = Question::find($id);
            if ($question->created_by == creatorId() && $question->workspace == getActiveWorkSpace()) {
