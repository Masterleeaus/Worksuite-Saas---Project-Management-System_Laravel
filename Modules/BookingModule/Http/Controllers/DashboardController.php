...
class DashboardController extends Controller
{
    ...
    {
        if (\Modules\BookingModule\Support\AppointmentPermission::check(Auth::user(), 'appointment dashboard manage')) {
            $userObj          = Auth::user();
            $events = [];
            ...
            $appointment_events = \Modules\BookingModule\Entities\Schedule::where('created_by', '=', creatorId())->where('workspace', getActiveWorkSpace())->where('status', '=', 'Approved');
            ...
            $unassignedCount = Schedule::where('workspace', getActiveWorkSpace())
                ->where(function ($q) {
                    $q->whereNull('assigned_to')->orWhere('assignment_status', 'unassigned');
                })
                ->count();
            $myCount = Schedule::where('workspace', getActiveWorkSpace())
                ->where('assigned_to', Auth::id())
                ->count();

            return view('bookingmodule::index', compact('currentWorkspace', 'totalAppointments', 'totalArrpvoed', 'totalReject', 'appointments', 'events', 'totalPending', 'arrProcessPer', 'unassignedCount', 'myCount'));
        }
    }
