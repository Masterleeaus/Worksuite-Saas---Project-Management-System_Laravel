...
use Modules\BookingModule\Http\Requests\PublicStoreBookingRequest;
use Illuminate\Routing\Controller;
use Illuminate\Support\Facades\Crypt;
use Illuminate\Support\Str;
use Illuminate\Support\Facades\Mail;
use Modules\BookingModule\Mail\ScheduleEmailVerificationMail;
use Modules\BookingModule\Services\PublicCaptchaVerifier;
...
use Modules\BookingModule\Events\AppointmentCallbackEvent;
use Modules\BookingModule\Events\CreateAppointments;
use Modules\BookingModule\Services\AutoAssign\ScheduleAutoAssignEngine;
...
class PublicAppointmentController extends Controller
{
    ...
    public function index($slug, $unique_id)
    {
        ...
        if ($appointment) {
            return view('bookingmodule::schedule.show', compact('appointment', 'workspace', 'week_day'));
        } else {
            return redirect()->back()->with('error', __('Something is wrong'));
        }
        return view('bookingmodule::schedule.index');
    }
...
// Captcha hook (optional)
$captchaVerifier = new PublicCaptchaVerifier();
if (!$captchaVerifier->verify($request)) {
    return redirect()->back()->withInput()->with('error', __('bookingmodule::public.captcha_required'));
}
        ...
        if ($appointment == null || $appointment->is_enabled == 'off') {
            return view('bookingmodule::appointment.data_not_found', compact('workspace'));
        }
        // Standalone meeting types (no module dependencies).
        $meetings = ['In Person', 'Phone Call'];
        ...
        return view('bookingmodule::appointment.public_appointment', compact('question', 'workspace', 'appointment', 'available_answer', 'meetings'));
    }
    ...
    public function store(Request $request, $slug, $id)
    {
        ...
        // Auto-assign staff to the booking (Pass 2). Safe to run unauthenticated.
        try {
            $engine = new ScheduleAutoAssignEngine(new AppointmentSettingsService(), new ScheduleAssignmentService());
            $engine->maybeAutoAssign($schedule);
        } catch (\Throwable $e) {
            // Do not block public booking creation.
        }


// Email verification (optional)
if (config('bookingmodule.public.require_email_verification', false)) {
    try {
        $token = Str::random(60);
        $schedule->email_verification_token = $token;
        $schedule->email_verification_sent_at = now();
        $schedule->email_verified_at = null;
        // Optionally mark status
        if (isset($schedule->status) && in_array($schedule->status, ['scheduled','pending','new'], true)) {
            $schedule->status = 'pending_verification';
        }
        $schedule->save();

        $verifyUrl = url('public/' . $workspace->slug . '/appointments/verify/' . $token);
        Mail::to($schedule->email)->send(new ScheduleEmailVerificationMail($schedule, $verifyUrl));
    } catch (\Throwable $e) {
        // Never block booking creation if mail fails.
    }
}
        ...
        if (!empty($company_settings['Appointment Send']) && $company_settings['Appointment Send'] == true) {
            ...
            return view('bookingmodule::appointment.form_submit', compact('workspace'))->with('success', __('Appointment Created successfully.') . ((!empty($resp) && $resp['is_success'] == false && !empty($resp['error'])) ? '<br> <span class="text-danger">' . $resp['error'] . '</span>' : ''));
        } else {
            return view('bookingmodule::appointment.form_submit', compact('workspace'));
        }
    ...
    {
        $workspace = WorkSpace::where('slug', $slug)->first();
        return view('bookingmodule::appointment.search', compact('workspace'));
    }
...
public function verifyEmail($slug, $token)
{
    $workspace = WorkSpace::where('slug', $slug)->first();
    if (!$workspace) {
        return redirect('/')->with('error', __('bookingmodule::public.verified_invalid'));
    }

    $schedule = Schedule::where('email_verification_token', $token)->first();
    if (!$schedule) {
        return redirect('/')->with('error', __('bookingmodule::public.verified_invalid'));
    }

    $schedule->email_verified_at = now();
    $schedule->email_verification_token = null;
    $schedule->save();

    return redirect('/')->with('success', __('bookingmodule::public.verified_success'));
}
