    ...
    public function up()
    {
        ...
        if (!Schema::hasTable('purchase_inventory_files')) {
            Schema::create('purchase_inventory_files', function (Blueprint $table) {
                ...
                $table->unsignedInteger('added_by')->nullable()->index('purchase_inventory_files_added_by_index');
                $table->unsignedInteger('last_updated_by')->nullable()->index('purchase_inventory_files_last_updated_by_index');
                $table->foreign(['added_by'])->references(['id'])->on('users')->onUpdate('CASCADE')->onDelete('SET NULL');
                $table->foreign(['last_updated_by'])->references(['id'])->on('users')->onUpdate('CASCADE')->onDelete('SET NULL');
                $table->timestamps();
            });
