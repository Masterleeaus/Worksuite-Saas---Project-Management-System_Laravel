...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h2>Log Activity – Order: {{ $order->name }}</h2>
    <a href="{{ route('fsmactivity.activities.index', $order->id) }}" class="btn btn-outline-secondary">Back</a>
</div>

<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('fsmactivity.activities.store', $order->id) }}">
            @csrf
            ...
            <div class="mb-3">
                ...
                <select name="state" class="form-select">
                    @foreach(\Modules\FSMActivity\Models\FSMActivity::STATES as $val => $label)
                        <option value="{{ $val }}" {{ old('state', 'open') === $val ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
