    ...
    public function up(): void
    {
        ...
        Schema::table('performance_settings', function (Blueprint $table) {
            if (! Schema::hasColumn('performance_settings', 'meeting_slack_notification')) {
                $table->enum('meeting_slack_notification', ['yes', 'no'])->default('no')->after('send_email_notification')->nullable();
            }
            if (! Schema::hasColumn('performance_settings', 'meeting_push_notification')) {
                $table->enum('meeting_push_notification', ['yes', 'no'])->default('no')->after('meeting_slack_notification')->nullable();
            }
            if (! Schema::hasColumn('performance_settings', 'meeting_email_notification')) {
                $table->enum('meeting_email_notification', ['yes', 'no'])->default('no')->after('meeting_push_notification')->nullable();
            }
