@php
    ...
    $customizerOptions = [
        ...
        'colorMainGradientFrom' => [
            'title' => __('Gradient Start Color'),
            'cssVar' => '--gradient-from',
            'type' => 'color',
            'values' => [],
            'showInCustomizer' => false,
        ],
        'colorMainGradientVia' => [
            'title' => __('Gradient Middle Color'),
            'cssVar' => '--gradient-via',
            'type' => 'color',
            'values' => [],
            'showInCustomizer' => false,
...
>
    ...
    >
        <x-slot:trigger
            ...
            href="#"
        >
            <span class="inline-block size-6 rounded-full bg-gradient-to-r from-gradient-from via-gradient-via to-gradient-to shadow-sm"></span>
        </x-slot:trigger>

        <x-slot:modal>
            <div class="space-y-2">
                @foreach (['colorMainGradientFrom', 'colorMainGradientVia', 'colorMainGradientTo'] as $key)
                    <label
                    ...
                    >
                        <span
                            ...
                            style="background-color: hsl(var({{ $customizerOptions[$key]['cssVar'] }}));"
                        >
                            <input
                                class="absolute inset-0 size-full cursor-pointer opacity-0"
                                id="lqd-customizer-{{ $key }}"
    ...
    <div class="relative">
        <x-button
            ...
            href="#"
            @click.prevent="if ( confirm('{{ __('Are you sure you want to discard changes?') }}') ) { discardChanges() }"
        >
...
@push('script')
    <script>
        ...
        window.lqdSaveUrl = "{{ route('titantheme.customizer.apply') }}";
    </script>
