...
class PostPaymentToWorksuiteAction
{
    public function execute(ZeroPayAttempt $attempt, array $payload = []): Payment
    {
        return DB::transaction(function () use ($attempt, $payload) {
            ...
            if ($attempt->processed_payment_id) {
                $existing = Payment::query()->find($attempt->processed_payment_id);
