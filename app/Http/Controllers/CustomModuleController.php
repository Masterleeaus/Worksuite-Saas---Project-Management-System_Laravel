...
use App\Support\ModuleDoctor\ModuleReadinessMatrixService;
use App\Support\ModuleDoctor\ModuleFixPlanBuilder;
use App\Services\CustomModules\ModuleInstallSnapshot;
use App\Services\CustomModules\CustomModuleInstallerState;
...
class CustomModuleController extends Controller
{
    ...
    public function index()
    {
        ...
        $this->doctorSummaryCards = [
            ['title' => 'Doctor summary', 'body' => 'Route truth, sidebar inclusion, permission readiness, compatibility checks, and tenant entitlement checks are enabled in the analyzer.', 'theme' => 'primary'],
    ...
    public function uploadTemp(Request $request)
    {
        ...
        $updateFilePath = config('froiden_envato.tmp_path');
        File::ensureDirectoryExists($updateFilePath);

        $stored = app(CustomModuleInstallerState::class)->storeUpload($request->file('file'), $updateFilePath);
        $files = collect(File::files($updateFilePath))
        ...
        return Reply::dataOnly([
            'status' => 'success',
            'message' => 'ZIP uploaded. Review it below, then run Analyze ZIP or Install if Ready.',
            'file' => $stored,
            'files' => $files,
            'reload' => true,
        ]);
    ...
    public function store(Request $request)
    {
        ...
        $this->syncModuleCatalogRecord($moduleName, $analysis, [
            'is_installed' => true,
            'module_path' => base_path('Modules/' . $moduleName),
            'install_source' => 'custom-installer',
        ]);
        if (class_exists(InstallationEventDispatcher::class)) {
        ...
        $finalize = $this->finalizeInstalledModuleState($moduleName, $analysis, $preInstallSnapshot ?? []);
        $analysis = $finalize['analysis'];
        if (!($finalize['ok'] ?? false)) {
            if (class_exists(InstallationEventDispatcher::class)) {
                InstallationEventDispatcher::installationFailed($moduleName, ['reason' => $finalize['message'] ?? 'Post-install validation failed']);
            }

            $analysis['diagnostics_html'] = $this->renderDiagnosticsHtml($analysis);
            $this->persistInstallLog('install_failed_post_validation', $analysis, $filePath);

            return Reply::dataOnly([
                'status' => 'fail',
                'message' => $finalize['message'] ?? 'Installation validation failed. Rollback executed.',
                'diagnostics' => $analysis['diagnostics_html'] ?? null,
                'analysis' => $analysis,
            ]);
        }
    ...
    private function auditInstalledModuleState(string $moduleName): array
    {
        ...
        if ($audit['module_registry_row'] === false) {
            $failed[] = 'module_registry';
        }

        $detail = 'Path: ' . ($audit['module_path_exists'] ? 'yes' : 'no')
            . '; discovered: ' . ($audit['module_discovered'] ? 'yes' : 'no')
            . '; enabled: ' . ($audit['module_enabled'] ? 'yes' : 'no')
            . '; registry: ' . (($audit['module_registry_row'] === null) ? 'n/a' : ($audit['module_registry_row'] ? 'yes' : 'no'))
    ...
    private function finalizeInstalledModuleState(string $moduleName, array $analysis, array $preInstallSnapshot = []): array
    {
        $auditResult = $analysis['post_install']['post_install_audit'] ?? $this->auditInstalledModuleState($moduleName);
        $failedChecks = collect($auditResult['failed_checks'] ?? [])->filter()->values()->all();

        $catalogSync = $this->syncModuleCatalogRecord($moduleName, $analysis, [
            'is_installed' => empty($failedChecks),
            'is_discovered' => !in_array('discovery', $failedChecks, true),
            'is_enabled' => !in_array('enabled', $failedChecks, true),
            'last_health_status' => empty($failedChecks) ? 'healthy' : 'failed',
            'last_doctor_run_at' => now(),
            'last_scanned_at' => now(),
        ]);

        $analysis['post_install']['module_catalog_sync'] = $catalogSync;

        if (empty($failedChecks)) {
            return ['ok' => true, 'analysis' => $analysis];
        }

        $analysis['blocking_issues'][] = 'Post-install validation failed: ' . implode(', ', $failedChecks) . '.';
        $analysis['post_install']['rollback'] = $this->rollbackInstalledModule($moduleName, $preInstallSnapshot);
        $this->syncModuleCatalogRecord($moduleName, $analysis, [
            'is_installed' => false,
            'is_enabled' => false,
            'last_health_status' => 'rollback',
            'last_doctor_run_at' => now(),
        ]);

        return [
            'ok' => false,
            'message' => 'Installation validation failed after file copy (' . implode(', ', $failedChecks) . '). Rollback executed.',
            'analysis' => $analysis,
        ];
    }

    private function rollbackInstalledModule(string $moduleName, array $preInstallSnapshot = []): array
    {
        try {
            if (!empty($preInstallSnapshot)) {
                app(ModuleInstallSnapshot::class)->restoreFrom($moduleName, $preInstallSnapshot);
                return ['status' => 'pass', 'detail' => 'Previous snapshot restored.'];
            }

            File::deleteDirectory(base_path('Modules/' . $moduleName));
            return ['status' => 'pass', 'detail' => 'New module files removed.'];
        } catch (\Throwable $e) {
            return ['status' => 'warn', 'detail' => 'Rollback could not complete cleanly: ' . $e->getMessage()];
        }
    }

    private function syncModuleCatalogRecord(string $moduleName, array $analysis = [], array $overrides = []): array
    {
        if (!Schema::hasTable('modules')) {
            return ['status' => 'warn', 'detail' => 'modules table not found; catalog sync skipped.'];
        }

        $payload = [
            'module_name' => $moduleName,
            'description' => Str::limit((string) ($analysis['description'] ?? $analysis['package_summary']['summary'] ?? 'Installed via custom module installer.'), 191, ''),
            'updated_at' => now(),
        ];

        $columnMap = [
            'slug' => Str::slug($moduleName),
            'display_name' => (string) ($analysis['display_name'] ?? Str::headline($moduleName)),
            'provider_class' => (string) ($analysis['provider_class'] ?? ''),
            'module_path' => (string) ($overrides['module_path'] ?? base_path('Modules/' . $moduleName)),
            'installed_version' => (string) ($analysis['version'] ?? $analysis['package_summary']['declared_version'] ?? 'unknown'),
            'available_version' => (string) ($analysis['version'] ?? $analysis['package_summary']['declared_version'] ?? 'unknown'),
            'is_installed' => (bool) ($overrides['is_installed'] ?? true),
            'is_discovered' => (bool) ($overrides['is_discovered'] ?? true),
            'is_enabled' => (bool) ($overrides['is_enabled'] ?? (Module::find($moduleName)?->isEnabled() ?? false)),
            'is_visible_in_packages' => (bool) ($overrides['is_visible_in_packages'] ?? true),
            'is_assignable_to_plans' => (bool) ($overrides['is_assignable_to_plans'] ?? true),
            'has_migrations' => (bool) ($overrides['has_migrations'] ?? !empty($analysis['signals']['has_migrations'] ?? ($analysis['migration_files'] ?? null))),
            'has_menu' => (bool) ($overrides['has_menu'] ?? !empty($analysis['signals']['has_menu'] ?? false)),
            'health_status' => (string) ($overrides['last_health_status'] ?? $overrides['health_status'] ?? 'healthy'),
            'install_source' => (string) ($overrides['install_source'] ?? 'custom-installer'),
            'last_scanned_at' => $overrides['last_scanned_at'] ?? now(),
            'last_doctor_run_at' => $overrides['last_doctor_run_at'] ?? now(),
            'last_enabled_at' => (($overrides['is_enabled'] ?? null) === true) ? now() : null,
            'auto_enable_on_install' => true,
            'category' => (string) ($analysis['detected_type'] ?? 'custom'),
            'icon' => (string) ($analysis['icon'] ?? 'ti ti-package'),
        ];
        ...
        $existing = DB::table('modules')->where('module_name', $moduleName)->first();

        if ($existing) {
            DB::table('modules')->where('module_name', $moduleName)->update($payload);
            return ['status' => 'pass', 'detail' => 'Updated modules catalog row.'];
        }

        $payload['created_at'] = now();
        DB::table('modules')->insert($payload);
        return ['status' => 'pass', 'detail' => 'Created modules catalog row.'];
    }
    ...
    private function analyzeUploadedModule($filePath, $prepareForInstall = false, $autoRepairMetadata = true)
    {
        ...
        $analysis['readiness_matrix'] = app(ModuleReadinessMatrixService::class)->build($analysis);
        $analysis['ordered_fix_plan'] = app(ModuleFixPlanBuilder::class)->build($analysis, $analysis['readiness_matrix']);
    ...
    private function resolveExtractedModulePath($moduleName)
    {
        ...
        while ($queue) {
            ...
            foreach (File::directories($path) as $directory) {
                $hasManifest = File::exists($directory . '/module.json') || File::exists($directory . '/Config/config.php');
                if ($hasManifest) {
                    $score = 0;
                    if (strcasecmp(basename($directory), (string) $moduleName) === 0) {
                        $score += 100;
                    }
                    ...
                    if (File::exists($manifestPath)) {
                        ...
                        if (is_array($manifest)) {
                            if (strcasecmp((string) ($manifest['name'] ?? ''), (string) $moduleName) === 0) {
                                $score += 50;
                            }
                    ...
                    $candidates[] = ['path' => $directory, 'score' => $score, 'depth' => $depth];
                }

                if ($depth < 2) {
                    $queue[] = [$directory, $depth + 1];
                }
            }
        }
...
private function detectPackageNameColumn(): ?string
{
    ...
    private function renderDiagnosticsHtml(array $analysis)
    {
        ...
        if (!empty($analysis['readiness_matrix']) && is_array($analysis['readiness_matrix'])) {
            $matrix = $analysis['readiness_matrix'];
            $html .= '<div class="alert alert-light border"><strong>Readiness matrix</strong><div class="small mt-2">Score: ' . e((string) ($matrix['score'] ?? 0)) . '% — ' . e($matrix['summary'] ?? '') . '</div><ul class="mb-0 mt-2">';
            foreach (($matrix['flags'] ?? []) as $flag => $status) {
                $html .= '<li><strong>' . e(str_replace('_', ' ', (string) $flag)) . ':</strong> ' . e(strtoupper((string) $status)) . '</li>';
            }
            $html .= '</ul></div>';
        }
        ...
        if (!empty($analysis['ordered_fix_plan']) && is_array($analysis['ordered_fix_plan'])) {
            $html .= '<div class="alert alert-light border"><strong>Ordered fix plan</strong><ol class="mb-0 mt-2">';
            foreach ($analysis['ordered_fix_plan'] as $repair) {
                $html .= '<li><strong>' . e($repair['title'] ?? 'Repair') . ':</strong> ' . e($repair['detail'] ?? '') . ' <span class="badge badge-light border ml-1">' . e(strtoupper((string) ($repair['risk'] ?? 'safe'))) . '</span></li>';
            }
            $html .= '</ol></div>';
        }
    ...
    private function persistInstallLog(string $event, array $analysis, ?string $filePath = null)
    {
        try {
            ...
            return ModuleInstallLog::create([
                ...
                'status' => $this->mapInstallLogStatus($analysis),
                'blocking_issues' => $analysis['blocking_issues'] ?? [],
                'warnings' => $analysis['warnings'] ?? [],
                'checks' => $analysis['checks'] ?? [],
                'package_summary' => $analysis['package_summary'] ?? [],
                'manifest_data' => $analysis['package_summary']['manifest'] ?? [],
                'validation_results' => [
                    'doctor_report' => $analysis['doctor_report'] ?? [],
                    'visibility_report' => $analysis['visibility_report'] ?? [],
                ],
                'validation_score' => (int) ($analysis['readiness_matrix']['score'] ?? 0),
                'readiness_flags' => $analysis['readiness_matrix']['flags'] ?? [],
                'readiness_score' => (int) ($analysis['readiness_matrix']['score'] ?? 0),
                'fix_plan' => $analysis['ordered_fix_plan'] ?? ($analysis['repair_plan'] ?? []),
                'repair_results' => [
                    'repair_queue' => $analysis['repair_queue'] ?? [],
                    'risk_profile' => $analysis['risk_profile'] ?? [],
                ],
                'applied_repairs' => collect($analysis['auto_repairs'] ?? [])->pluck('title')->filter()->values()->all(),
                'pre_install_snapshot' => $analysis['pre_install_snapshot'] ?? null,
                'can_rollback' => !empty($analysis['pre_install_snapshot']),
                'installed_at' => now(),
                'last_verified_at' => now(),
    ...
    private function mapInstallLogStatus(array $analysis): string
    {
        return match ($analysis['installability_status'] ?? 'blocked') {
            'ready' => 'installed',
            'upgrade-candidate' => 'partial',
            'installable-with-warnings' => 'partial',
            'overlay-only' => 'failed',
            default => 'failed',
        };
    }
    ...
    public function validateModule($moduleName)
    {
        ...
        $wrongMessage = 'The ZIP that you are trying to install is not a compatible Worksuite module.';
        ...
        $moduleJsonPath = $modulePath . '/module.json';
        $extensionJsonPath = $modulePath . '/extension.json';
        $configPath = $modulePath . '/Config/config.php';

        if (file_exists($extensionJsonPath) && !file_exists($moduleJsonPath) && !file_exists($configPath)) {
            return [
                'status' => false,
                'message' => 'This ZIP looks like a MagicAI extension package, not a Worksuite module package.'
            ];
        }
        ...
        if (file_exists($moduleJsonPath)) {
            ...
            if (!is_array($decoded)) {
                return [
                    'status' => false,
                    'message' => 'module.json exists but is not valid JSON.'
                ];
            }

            $moduleJson = $decoded;

            $manifestName = (string) ($moduleJson['name'] ?? '');
            if ($manifestName !== '' && strcasecmp($manifestName, (string) $moduleName) !== 0) {
                return [
                    'status' => false,
                    'message' => 'The extracted module folder does not match module.json.'
                ];
            }

            $providers = array_values(array_filter((array) ($moduleJson['providers'] ?? []), fn ($provider) => is_string($provider) && $provider !== ''));
            if (empty($providers)) {
                return [
                    'status' => false,
                    'message' => 'module.json is missing providers. This does not look like a valid nWidart module.'
                ];
            }

            if (array_key_exists('files', $moduleJson) && !is_array($moduleJson['files']) && !is_null($moduleJson['files'])) {
                return [
                    'status' => false,
                    'message' => 'module.json has an invalid files value. It must be an array or null.'
                ];
            }
        }

        if (!file_exists($moduleJsonPath) && !file_exists($configPath)) {
            return [
                'status' => false,
        ...
        if (file_exists($configPath)) {
            ...
            if (($config['parent_envato_id'] ?? null) !== config('froiden_envato.envato_item_id')) {
                return [
                    'status' => false,
                    'message' => 'You are installing the wrong module for this Worksuite product.'
                ];
            }

            if (($config['parent_product_name'] ?? null) !== config('froiden_envato.envato_product_name')) {
                return [
                    'status' => false,
                    'message' => 'This module targets a different parent product.'
                ];
            }

            if (!isset($config['parent_min_version'])) {
                $errorMessage = App::environment('codecanyon')
                    ? 'Please download and install the latest version of the module.'
                    : 'Minimum version of <b>' . $appName . ' main application</b> is not defined in the module.';
            ...
            if (File::exists(base_path('version.txt')) && version_compare(trim(File::get(base_path('version.txt'))), (string) $config['parent_min_version'], '<')) {
                return [
                    'status' => false,
                    'message' => 'Minimum version of <b>' . $appName . ' main application</b> should be greater than or equal to <b>' . $config['parent_min_version'] . '</b>. Your application version is <b>' . File::get(base_path('version.txt')) . '</b>'
                ];
            }
        }

        $requires = (array) ($moduleJson['requires'] ?? []);
        $worksuiteRequirement = $requires['worksuite'] ?? $requires['worksuite_version'] ?? $requires['core'] ?? null;

        if ($worksuiteRequirement && File::exists(base_path('version.txt'))) {
            $currentVersion = trim(File::get(base_path('version.txt')));
            if (preg_match('/^(>=|>|<=|<|=)?\s*([0-9][A-Za-z0-9.\-]*)$/', (string) $worksuiteRequirement, $matches)) {
                $operator = $matches[1] ?: '>=';
                $requiredVersion = $matches[2];
                if (!version_compare($currentVersion, $requiredVersion, $operator)) {
                    return [
                        'status' => false,
                        'message' => 'This module requires Worksuite version ' . $worksuiteRequirement . '. Current version: ' . $currentVersion . '.'
                    ];
                }
            }
        }
