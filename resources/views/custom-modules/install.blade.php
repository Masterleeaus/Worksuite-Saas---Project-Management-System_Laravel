        ...
        <x-setting-card>
            ...
            <div class="col-lg-12 col-md-12 ntfcn-tab-content-left w-100 p-4 ">
                ...
                <div class="alert alert-light border mt-3 mb-0">
                    <strong>User menu visibility checks are now enabled. Route truth, sidebar inclusion, permission DB readiness, role-pivot truth, runtime/module registry truth, and tenant package entitlement checks now run in the analyzer.</strong> The installer will warn if the module can install but still stay hidden from tenant users due to sidebar, route, permission, role-mapping, runtime-registry, module_settings, company_module_settings, or package bridge problems.
                </div>
...
@push('scripts')
    <script>
        ...
        $(document).ready(function () {
            const uploadFile = "{{ route('custom-modules.upload-temp') }}";
            ...
            myDrop.on("success", function (file, response) {
                if (response && response.status === 'success') {
                    $('#install-process').html('<div class="alert alert-success">' + (response.message || 'ZIP uploaded successfully.') + '</div>');
                    setTimeout(function () {
            ...
            myDrop.on("error", function (file, response) {
                ...
                $('#install-process').html('<div class="alert alert-danger">' + message + '</div>');
            });
        });
