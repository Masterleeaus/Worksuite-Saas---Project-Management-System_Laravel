...
@foreach([
 ...
 ['label'=>'Avg Readiness','value'=>($readinessSummary['average_score'] ?? 0).'%'],
] as $card)
...
<div class="col-md-3 mb-3"><div class="card border h-100"><div class="card-header">Compatibility</div><div class="card-body"><div class="small text-muted">Manifest / target / rejection policy</div><div class="h4">{{ ($compatibilitySummary['manifest_required'] ?? false) ? 'module.json' : 'mixed' }}</div><div class="small mt-2">Target: {{ $compatibilitySummary['nwidart_target'] ?? '/Modules' }} · Reject non-Worksuite manifests: {{ ($compatibilitySummary['non_worksuite_manifest_rejected'] ?? false) ? 'yes' : 'no' }}</div></div></div></div>
...
<div class="col-md-3 mb-3"><div class="card border h-100"><div class="card-header">Readiness Engine</div><div class="card-body"><div class="small text-muted">Scored installation audits</div><div class="h4">{{ $readinessSummary['average_score'] ?? 0 }}%</div><div class="small mt-2">Pass: {{ $readinessSummary['pass_count'] ?? 0 }} · Warn: {{ $readinessSummary['warn_count'] ?? 0 }} · Fail: {{ $readinessSummary['fail_count'] ?? 0 }}</div><div class="small mt-2">Fix steps pending: {{ $readinessSummary['fix_steps_pending'] ?? 0 }}</div></div></div></div>
</div>
<div class="row">
